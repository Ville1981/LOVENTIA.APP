...
// --- REPLACE START: sitemap.xml static route ---
/**
 * sitemap.xml:
 * Serves the pre-generated client/public/sitemap.xml so that
 * http://localhost:5000/sitemap.xml is available for smoke tests and crawlers.
 *
 * The file is generated by client/scripts/generate-sitemap.mjs during the
 * client build (prebuild hook).
 *
 * We resolve the file using a couple of common layouts:
 *  - repo-root/client/public/sitemap.xml (when server is started from repo root)
 *  - ../client/public/sitemap.xml       (when server is started from server/)
 */
const sitemapCandidateFromServerCwd = resolve(
  process.cwd(),
  "..",
  "client",
  "public",
...
const sitemapCandidateFromRepoCwd = resolve(
  process.cwd(),
  "client",
  "public",
  "sitemap.xml"
...
const SITEMAP_FILE =
  (existsSync(sitemapCandidateFromServerCwd) &&
    sitemapCandidateFromServerCwd) ||
  (existsSync(sitemapCandidateFromRepoCwd) && sitemapCandidateFromRepoCwd) ||
  sitemapCandidateFromServerCwd;
...
app.get("/sitemap.xml", (req, res, next) => {
  ...
  res.sendFile(SITEMAP_FILE, (err) => {
    ...
    if (err.code === "ENOENT") {
      return res
        .status(404)
        .json({ error: "sitemap.xml not found on server filesystem" });
    }
    return next(err);
  });
});

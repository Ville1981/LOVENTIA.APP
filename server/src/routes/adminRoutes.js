// PATH: server/src/routes/adminRoutes.js

// --- REPLACE START: unified, protected admin routes (metrics, stripe revenue, CSV exports) ---
// This file resolves previous merge conflicts (<<<<<<, =======, >>>>>>>) and preserves functionality.
// All comments are in English. The router stays ESM-compatible and guarded for admin-only access.

import express from "express";
import mongoose from "mongoose";

// Auth / role guards (assumes upstream authentication populates req.user)
import authenticate from "../middleware/authenticate.js";
import requireAdmin from "../middleware/requireAdmin.js";
// Keep roleAuthorization import available if you later want extra role checks.
// Not used directly here to avoid double-guarding and to prevent duplicate logic.
...
/* =============================================================================
   ...
   - If the module is missing, provide safe inline fallbacks so the app runs.
============================================================================= */
let getMetrics;
...
try {
  ...
  getStripeRevenue = async (_req, res) => {
    try {
      const BillingEvents = mongoose.connection.collection("billing_events");
      if (!BillingEvents) {
        return res
          .status(200)
          .json({ revenueLast30d: 0, currency: "usd", note: "billing_events collection not found" });
      }
      ...
      // NOTE: Stripe amounts are usually in cents. Convert to standard units.
      // Try a few common paths: amount_paid, total, amount_total.
...
/* =============================================================================
   ...
   - Metrics + Stripe revenue (from controller or fallback)
============================================================================= */
...
============================================================================= */
router.get("/export/users.csv", authenticate, requireAdmin, async (_req, res) => {
  try {
...
/* =============================================================================
   CSV: Matches
============================================================================= */
router.get("/export/matches.csv", authenticate, requireAdmin, async (_req, res) => {
  try {
...
============================================================================= */
router.get("/export/billing-events.csv", authenticate, requireAdmin, async (_req, res) => {
  try {
    const BillingEvents = mongoose.connection.collection("billing_events");
...
============================================================================= */
router.get("/export/billing-subscriptions.csv", authenticate, requireAdmin, async (_req, res) => {
  try {
    const users = await User.find(
      {},
      {

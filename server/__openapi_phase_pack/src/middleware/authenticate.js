 ...
 *
 * Implementation note:
 * - This file prioritizes clarity. Comments are verbose on purpose to preserve
 *   maintainability and help future debugging without hunting external docs.
 */
...
/**
 * Returns the first defined, non-empty value in the provided arguments.
 ...
 *
 * CRITICAL: This middleware NEVER returns 404. Only 401 (auth problems) or
 * 500 (server misconfiguration) where justified. 404s are handled by the
 * app-level notFound handler.
 */
export default function authenticate(req, res, next) {
  try {
    ...
    // We check path WITHOUT the '/api' prefix, since this middleware is usually
    // mounted after `app.use('/api', routes)` and routers see the trimmed path.
    const path = req.path || "";
    if (
    ...
    if (!secretsToTry.length) {
      // Explicit 500 here is intentional to flag misconfig immediately
      return res
        .status(500)
    ...
    if (!decoded) {
      if (process.env.NODE_ENV !== "production") {
        // Keep this console.warn; helps diagnose mismatched secrets in dev
        console.warn(

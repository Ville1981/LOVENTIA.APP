...
export const login = async (req, res, next) => {
  try {
    ...
    // Ensures the cookie is only sent by the browser and not accessible via JavaScript,
    // and allows cross-site usage (needed if front-end runs on a different domain).
...
export const refreshToken = async (req, res, next) => {
  try {
    ...
    jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (err, payload) => {
      ...
      // If you want to issue a fresh refresh token on each call, uncomment:
...
export const logout = (req, res) => {
  // --- REPLACE START: clear the refreshToken cookie using centralized options ---

...
const EducationReligionImportanceFields = ({ education, setEducation, religionImportance, setReligionImportance, t }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
    <div>
      <label className="block font-medium">🎓 {t("profile.education")}</label>
      <select
        value={education}
      ...
      >
        ...
        <option value="Alempi korkeakoulututkinto">{t("profile.lowerUni")}</option>
        <option value="Ylempi korkeakoulututkinto">{t("profile.upperUni")}</option>
        <option value="Tohtori">{t("profile.phd")}</option>
      </select>
    </div>
    <div>
      <label className="block font-medium">🕊 {t("profile.religionImportance")}</label>
      <select

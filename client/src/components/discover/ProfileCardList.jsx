...
// --- REPLACE START: ProfileCardList – stable ids + inline ad slots every N items (minimal changes) + likes quota passthrough + optional onBlock wiring ---
 ...
 * - Can forward likes quota information to ProfileCard → ActionButtons
 *   via likesLimitPerDay / likesRemainingToday props (optional).
 * - (NEW) Optionally forwards an onBlock callback so cards can trigger block-actions.
 */
const ProfileCardList = ({
  users = [],
  onAction,
  // (NEW) Optional likes quota data for free users – forwarded down to ActionButtons
  likesLimitPerDay,
  likesRemainingToday,
  // (NEW) Optional block callback – forwarded down to ProfileCard
  onBlock,
}) => {
  ...
  // Compute a stable content key: when the user set changes, this string changes
  ...
  const settings = useMemo(
    () => ({
      ...
      // Accessibility / focus behavior tuned for Discover usage (buttons handle focus)
      accessibility: false,
      focusOnSelect: false,
      focusOnChange: false,
      pauseOnFocus: false,
   ...
   * We keep keys stable and distinct to avoid React reconciliation issues.
   */
  const slides = useMemo(() => {
    ...
    for (let i = 0; i < safeUsers.length; i += 1) {
      ...
      // Always push the profile card itself.
      out.push({
        kind: "profile",
        key: `u-${u.id}`,
        user: u,
      ...
      // Decide whether to add an ad after this card.
      if (INLINE_EVERY > 0) {
        const isBoundary =
          (i + 1) % INLINE_EVERY === 0 && i !== safeUsers.length - 1;
        if (isBoundary) {
            ...
            return (
              ...
              >
                <ProfileCard
                  ...
                  // (NEW) If an onBlock handler is provided, forward it with this user's id.
                  onBlock={
                    typeof onBlock === "function" ? () => onBlock(u.id) : undefined
                  }
                />
              </div>
            );
...
ProfileCardList.propTypes = {
  ...
  // (NEW) Optional block callback – called with user id when card requests block

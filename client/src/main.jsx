...
* This injects a style tag only in DEV; production CSS remains untouched.
  */
  ...
  const css = `  /* Dev-only overlay guard */
...
root.render(
<React.StrictMode> <ConsentProvider> <QueryClientProvider client={queryClient}> <AuthProvider>
{/* Suspense can stay even if not strictly required */} <Suspense fallback={null}> <App />
{/* ConsentBanner is mounted inside App.jsx to avoid duplication */} </Suspense> </AuthProvider> </QueryClientProvider> </ConsentProvider>
</React.StrictMode>
);
}

// PATH: client/src/pages/ProfileHub.jsx
...
export default function ProfileHub() {
  // i18n translations: use real t function, no stubs
  ...
  if (!user) {
    // --- REPLACE START: loading text (fix ellipsis encoding) ---
    return <div className="text-center mt-12">Loading profile…</div>;
    // --- REPLACE END ---
  }

  // --- REPLACE START: derive profileUserId + emailVerified flag for badge in header ---
  const profileUserId =
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        {/* --- REPLACE START: profile header with email + emailVerified badge + Manage Photos button --- */}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold">
                {t("profile:hubTitle", "Your profile")}
              </h1>
              {isEmailVerified && (
                <span
                  className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800 border border-green-300"
                  title={t(
                    "profile:emailVerifiedBadgeTitle",
                    "Your email address has been verified"
                  )}
                  aria-label={t(
                    "profile:emailVerifiedBadgeAria",
                    "Email address verified"
                  )}
                >
                  ✓ {t("profile:emailVerifiedBadge", "Verified")}
                </span>
              )}
            </div>
            {user.email && (
        ...
        {/* --- REPLACE END: profile header with email + emailVerified badge + Manage Photos button --- */}

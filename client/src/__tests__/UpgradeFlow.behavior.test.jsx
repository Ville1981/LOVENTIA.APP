// PATH: client/src/__tests__/UpgradeFlow.behavior.test.jsx

// --- REPLACE START: Upgrade CTA routes to /settings/subscriptions from DiscoverFilters (deduplicated timer stub) ---
 ...
 * Keep a single patch to avoid duplicate symbol errors across test files.
 */
...
describe("Upgrade flow (CTA visibility + navigation)", () => {
  it("shows Upgrade CTA when dealbreakers are used by non-premium and navigates to subscriptions", async () => {
      ...
      <MemoryRouter initialEntries={["/"]}>
        <Routes>
          ...
          <Route
            ...
            element={
              <div data-testid="subscriptions-page">Subscriptions Page</div>
            }
          />
        </Routes>
      </MemoryRouter>
    ...
    const form = document.querySelector('[data-cy="DiscoverFilters__form"]');
